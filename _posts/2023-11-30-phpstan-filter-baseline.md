...
Now you can trigger your regular `vendor/bin/phpstan ...` command which no longer ignores the filtered errors.
That way you can work on the problems as you are used to based on PHPStan result list.

Alternatively to `--exclude` you can also use `--include` to filter the baseline list. This might be useful if you want to further process the filtered error list in a separate tool.
