...
if (PHP_VERSION_ID < 70400) {
    // should error about null coalescing operator, which requires PHP 7.4+
    $y = $_GET['y'] ?? [];
} else {
    $x = $_GET['x']?? [];
}
...
class MySocket
{
  public function create(): \Socket
  {
    ...
    // can only return `\Socket|false` but PHPStan sometimes mixes it up with PHP7 `resource` type
